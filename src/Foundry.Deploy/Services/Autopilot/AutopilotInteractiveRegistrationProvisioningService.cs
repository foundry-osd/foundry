    ...
    private static string BuildAutomaticLaunchScript()
    {
        return string.Join(
            ...
            [
                ...
                "$fallbackTrigger = New-ScheduledTaskTrigger -Once -At (Get-Date).AddSeconds(30)",
                "$administratorsSid = New-Object System.Security.Principal.SecurityIdentifier 'S-1-5-32-544'",
                "$administratorsGroup = $administratorsSid.Translate([System.Security.Principal.NTAccount]).Value",
                "Write-FoundryTaskLog -Message \"Resolved Administrators group as '$administratorsGroup'.\"",
                "$principal = New-ScheduledTaskPrincipal -GroupId $administratorsGroup -RunLevel Highest",
                "$settings = New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries -StartWhenAvailable",
                "$task = New-ScheduledTask -Action $action -Trigger @($logonTrigger, $fallbackTrigger) -Principal $principal -Settings $settings",
