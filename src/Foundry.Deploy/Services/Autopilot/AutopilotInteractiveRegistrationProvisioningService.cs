    ...
    private static string BuildForegroundWrapper()
    {
        return string.Join(
            ...
            [
                ...
                "        Write-FoundryForegroundLog -Message 'SendInput did not report all keyboard inputs as sent.'",
                "    }",
                "}",
                "function Wait-FoundryOobeCommandPrompt {",
                "    param(",
                "        [Parameter(Mandatory = $true)][int]$SessionId,",
                "        [Parameter(Mandatory = $true)][int[]]$ExistingCommandPromptIds",
                "    )",
                ...
                "    $existingCommandPromptIds = Get-FoundryCommandPromptIds -SessionId $sessionId",
