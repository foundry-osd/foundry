    ...
    private static string BuildAutomaticLaunchScript()
    {
        return string.Join(
            ...
            [
                ...
                "public static class FoundryActiveConsoleSession",
                "{",
                ...
                "    private static extern bool WTSQuerySessionInformation(IntPtr hServer, int sessionId, WtsInfoClass infoClass, out IntPtr buffer, out int bytesReturned);",
                ...
                "        {",
                "            throw new InvalidOperationException(\"No active console session is attached.\");",
                "        }",
                "        string userName = QueryString((int)sessionId, WtsInfoClass.WTSUserName);",
                "        string domainName = QueryString((int)sessionId, WtsInfoClass.WTSDomainName);",
                "        if (String.IsNullOrWhiteSpace(userName))",
                "        {",
                "            throw new InvalidOperationException(\"The active console session does not have a user name.\");",
                "        }",
                ...
                "}",
                "'@",
                "Add-Type -TypeDefinition $activeConsoleSource",
                "function Get-FoundryActiveConsoleUser {",
                "    return [FoundryActiveConsoleSession]::GetUserName()",
                "}",
                ...
                "$interactiveUser = Get-FoundryActiveConsoleUser",
                "Write-FoundryTaskLog -Message \"Resolved active console launch user as '$interactiveUser'.\"",
