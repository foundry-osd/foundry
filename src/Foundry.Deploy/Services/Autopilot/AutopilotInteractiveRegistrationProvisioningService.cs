    ...
    private static string BuildOobeWaiter()
    {
        return string.Join(
            ...
            [
                ...
                "$diagnosticProcessNames = @('CloudExperienceHost', 'CloudExperienceHostBroker', 'UserOOBEBroker', 'oobenetworkconnectionflow', 'ApplicationFrameHost', 'ShellExperienceHost', 'RuntimeBroker', 'wwahost', 'powershell', 'pwsh', 'cmd')",
                ...
                "        Write-FoundryOobeWaiterLog -Message \"Active console session $activeSessionId detected.\"",
                "        break",
                "    }",
    ...
    private static string BuildForegroundWrapper()
    {
        return string.Join(
            ...
            [
                ...
                "$oobeProcessNames = @('CloudExperienceHost', 'CloudExperienceHostBroker', 'UserOOBEBroker', 'oobenetworkconnectionflow', 'ApplicationFrameHost', 'wwahost')",
                "New-Item -Path $logRoot -ItemType Directory -Force | Out-Null",
                ...
                string.Empty,
                "        uint processId;",
                "        uint targetThreadId = GetWindowThreadProcessId(windowHandle, out processId);",
                "        uint currentThreadId = GetCurrentThreadId();",
                "        bool inputAttached = targetThreadId != 0 && targetThreadId != currentThreadId && AttachThreadInput(currentThreadId, targetThreadId, true);",
                "        try",
                "        {",
                ...
                "}",
                "function Get-FoundryOobeWindow {",
                "    $sessionId = Get-FoundryCurrentSessionId",
                "    $foregroundHandle = [FoundryOobeForegroundNativeMethods]::GetForegroundWindow()",
                "    if ($foregroundHandle -ne [IntPtr]::Zero) {",
                "        $foregroundProcessId = [uint32]0",
                "        [void][FoundryOobeForegroundNativeMethods]::GetWindowThreadProcessId($foregroundHandle, [ref]$foregroundProcessId)",
                "        $foregroundProcess = Get-Process -Id $foregroundProcessId -ErrorAction SilentlyContinue",
                "        if ($foregroundProcess -and $foregroundProcess.SessionId -eq $sessionId -and $foregroundProcess.ProcessName -in $oobeProcessNames) {",
                "            return [pscustomobject]@{ Handle = $foregroundHandle; Process = $foregroundProcess }",
                "        }",
                "    }",
                "    foreach ($processName in $oobeProcessNames) {",
                "        $oobeProcess = Get-Process -Name $processName -ErrorAction SilentlyContinue |",
                "            Where-Object { $_.SessionId -eq $sessionId -and $_.MainWindowHandle -ne 0 } |",
                "            Sort-Object Id |",
                "            Select-Object -First 1",
                "        if ($oobeProcess) {",
                "            return [pscustomobject]@{ Handle = [IntPtr]$oobeProcess.MainWindowHandle; Process = $oobeProcess }",
                "        }",
                "    }",
                "    return $null",
                "}",
                "function Invoke-FoundryActivateOobeWindow {",
                "    $oobeWindow = Get-FoundryOobeWindow",
                "    if ($oobeWindow -and [FoundryOobeForegroundNativeMethods]::ActivateWindow($oobeWindow.Handle)) {",
                "        Write-FoundryForegroundLog -Message \"Activated OOBE window owned by $($oobeWindow.Process.ProcessName) PID $($oobeWindow.Process.Id).\"",
                "        return $true",
                "    }",
                "    return $false",
                "}",
                ...
                "    Write-FoundryForegroundLog -Message 'Sending Shift+F10 to the active OOBE window.'",
                "    $sent = [FoundryOobeForegroundNativeMethods]::SendShiftF10()",
                "    if (-not $sent) {",
                ...
                "    return $sent",
                "}",
                "function Get-FoundryOobeCommandPrompt {",
                "    param(",
                ...
                "            $_.Id -notin $ExistingCommandPromptIds -and",
                "            $_.MainWindowHandle -ne 0",
                "        } |",
                "        Sort-Object Id |",
                "        Select-Object -First 1",
                "}",
                "function Wait-FoundryOobeForegroundAccess {",
                "    param(",
                "        [Parameter(Mandatory = $true)][int]$SessionId,",
                "        [int[]]$ExistingCommandPromptIds = @()",
                "    )",
                "    $foregroundDeadline = [DateTimeOffset]::UtcNow.AddMinutes(2)",
                "    $nextShiftF10Attempt = [DateTimeOffset]::MinValue",
                "    while ([DateTimeOffset]::UtcNow -lt $foregroundDeadline) {",
                "        $commandPrompt = Get-FoundryOobeCommandPrompt -SessionId $SessionId -ExistingCommandPromptIds $ExistingCommandPromptIds",
                "        if ($commandPrompt) {",
                "            return $commandPrompt",
                "        }",
                "        if ([DateTimeOffset]::UtcNow -ge $nextShiftF10Attempt -and (Invoke-FoundryActivateOobeWindow)) {",
                "            [void](Invoke-FoundryShiftF10)",
                "            $nextShiftF10Attempt = [DateTimeOffset]::UtcNow.AddSeconds(3)",
                "        }",
                ...
                "    $commandPrompt = Wait-FoundryOobeForegroundAccess -SessionId $sessionId -ExistingCommandPromptIds @($existingCommandPromptIds)",
                "    if (-not $commandPrompt) {",
                "        throw 'Timed out while waiting for OOBE foreground access.'",
                "    }",
