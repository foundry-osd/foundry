    ...
    private static string BuildOobeWaiter()
    {
        return string.Join(
            ...
            [
                ...
                string.Empty,
                "    [DllImport(\"oobenotification.dll\", SetLastError = true)]",
                ...
                "}",
                "function Test-FoundryOobeComplete {",
                "    try {",
                "        $isOobeComplete = $false",
                "        $success = [FoundryOobeNativeMethods]::OOBEComplete([ref]$isOobeComplete)",
                "        if (-not $success) {",
                "            Write-FoundryOobeWaiterLog -Message 'OOBEComplete returned false. Continuing to wait for active OOBE console session.'",
                "            return $false",
                "        }",
                "        return $isOobeComplete",
                "    }",
                "    catch {",
                "        Write-FoundryOobeWaiterLog -Message \"OOBEComplete check failed. $($_.Exception.Message)\"",
                "        return $false",
                "    }",
                "}",
                ...
                "        exit 0",
                "    }",
                "    if (Test-FoundryOobeComplete) {",
                "        Write-FoundryOobeWaiterLog -Message 'OOBE is already complete. Registration assistant will not be launched.'",
                "        exit 0",
                "    }",
