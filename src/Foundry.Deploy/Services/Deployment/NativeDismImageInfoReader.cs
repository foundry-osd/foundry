...
/// </remarks>
internal sealed class NativeDismImageInfoReader : IWindowsImageInfoReader, IDisposable
{
    ...
    private IReadOnlyList<WindowsImageInfo> Read(string imagePath, CancellationToken cancellationToken)
    {
        lock (_gate)
        {
    ...
    private static WindowsImageInfo[] CopyImageInfo(IntPtr buffer, uint count)
    {
        ...
        var images = new WindowsImageInfo[imageCount];
        for (int position = 0; position < images.Length; position++)
        {
            ...
            images[position] = new WindowsImageInfo(
                ...
                new Version(checked((int)image.MajorVersion), checked((int)image.MinorVersion), checked((int)image.Build)));
        }
