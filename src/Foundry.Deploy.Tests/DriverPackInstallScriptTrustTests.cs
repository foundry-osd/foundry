    ...
    public async Task Script_RequiresExactTrustedPublisher_AndLocksThroughExecution(string command, string family, string status, bool expectedPublisher, bool allowed)
    {
        ...
        const string harness = """
            ...
            function Invoke-FoundryInstaller {
                ...
                if ($TimeoutSeconds -ne 1800) { throw 'Native calls must have a bounded wait.' }
                Test-PackageLock; $launches.Add($FilePath); return 0
            }
            function Write-FoundryDriverOutcome { param($Outcome) }
            function Get-FoundryDriverPath { [pscustomobject]@{Exists=$true;Value='prior-value';Kind='ExpandString'} }
            function Set-FoundryDriverPath { param($State); if ($State.Value -ne 'C:\Drivers' -and ($State.Value -ne 'prior-value' -or $State.Kind -ne 'ExpandString')) { throw 'Prior registry state not restored.' } }
            ...
            try { & $env:FOUNDRY_TEST_SCRIPT -CommandKind $env:FOUNDRY_TEST_COMMAND -PackagePath $env:FOUNDRY_TEST_PACKAGE -ExpectedSha256 ([BitConverter]::ToString([Security.Cryptography.SHA256]::Create().ComputeHash([IO.File]::ReadAllBytes($env:FOUNDRY_TEST_PACKAGE))).Replace('-','')) -ExpectedSizeBytes 7; $accepted = $true }
            catch { Write-Output $_.Exception.Message; $accepted = $false }
