...
{
    [Fact]
    public void Provision_LaterWriteFailureRemovesOnlyNewOwnedSecrets()
    {
        string root = CreateWindowsRoot();
        var service = new PreOobeScriptProvisioningService(new SetupCompleteScriptService(),
            path => Directory.CreateDirectory(path), path =>
            {
                if (path.EndsWith("Invoke-FoundryPreOobe.ps1", StringComparison.Ordinal)) throw new IOException("fixture later write");
            });
        var script = CreateScript("network", "Import-NetworkProfiles.ps1", PreOobeScriptPriority.NetworkProfileImport) with
        {
            DataFiles = [new() { FileName = "secret.txt", Content = "secret", IsSensitive = true,
                CleanupDisposition = PreOobeCleanupDisposition.SecretAlways }]
        };
        IOException failure = Assert.Throws<IOException>(() => service.Provision(root, [script]));
        Assert.Equal("fixture later write", failure.Message);
        Assert.False(File.Exists(Path.Combine(root, "Windows", "Temp", "Foundry", "PreOobe", "Data", "secret.txt")));
    }
    ...
    public void RestrictedDescriptor_RejectsOrdinaryOwnerDespiteRestrictedDacl(string owner, bool valid)
    {
        var descriptor = new FileSecurity();
        descriptor.SetOwner(new SecurityIdentifier(owner));
        foreach (string sid in new[] { "S-1-5-18", "S-1-5-32-544" })
            descriptor.AddAccessRule(new FileSystemAccessRule(new SecurityIdentifier(sid), FileSystemRights.FullControl, AccessControlType.Allow));
        if (valid) SensitiveStagingPolicy.Verify(descriptor, false);
        else Assert.Throws<UnauthorizedAccessException>(() => SensitiveStagingPolicy.Verify(descriptor, false));
    }

    [Fact]
    public void Provision_RequiresRestrictedDirectoryBeforeAnySecretBytes()
    {
        string root = CreateWindowsRoot();
        var service = new PreOobeScriptProvisioningService(new SetupCompleteScriptService(),
            _ => throw new UnauthorizedAccessException("network_secret_acl_failed"), _ => { });
        var script = CreateScript("network-profile-roaming", "Import-NetworkProfiles.ps1", PreOobeScriptPriority.NetworkProfileImport) with
        {
            DataFiles = [new() { FileName = "secret.txt", Content = "secret", IsSensitive = true,
                OwningActionId = "network-profile-roaming", CleanupDisposition = PreOobeCleanupDisposition.SecretAlways }]
        };
        Assert.Throws<UnauthorizedAccessException>(() => service.Provision(root, [script]));
        Assert.False(File.Exists(Path.Combine(root, "Windows", "Temp", "Foundry", "PreOobe", "Data", "secret.txt")));
    }
    ...
    public void Provision_OrdersScriptsByPriorityThenId()
    {
        ...
        var service = new PreOobeScriptProvisioningService(new SetupCompleteScriptService(), path => Directory.CreateDirectory(path), _ => { });
        ...
        string runner = File.ReadAllText(result.ManifestPath);
    ...
    public void Provision_StagesEmbeddedCleanupScript()
    {
        ...
        Assert.DoesNotContain("C:\\DRIVERS", stagedScript);
        Assert.DoesNotContain("C:\\Drivers", stagedScript);
        Assert.Contains("Remove-FoundryActionInputs", stagedScript);
        Assert.NotEmpty(stagedScript);
    }
    ...
    public void Provision_KeepsRootFolderCleanupOutOfDriverScript()
    {
        ...
        Assert.DoesNotContain("C:\\Drivers", cleanupScript);
    }
    ...
    public void Provision_StagesNestedBinaryDataFiles()
    {
        ...
        Assert.Contains(
            ...
            manifest.RootElement.GetProperty("scripts")[0].GetProperty("dataFiles")[0].GetProperty("fileName").GetString());
    }
    ...
    public void Provision_WritesSetupCompleteLauncherBlock()
    {
        ...
        Assert.Contains("exit /b %FOUNDRY_PREOOBE_EXIT%", setupComplete);
