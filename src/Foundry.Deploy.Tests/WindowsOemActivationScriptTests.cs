...
{
    [Theory]
    [InlineData("default", "Windows OEM activation succeeded.", "InstallProductKey,Activate")]
    [InlineData("home", "Windows OEM activation succeeded.", "InstallProductKey,Activate")]
    [InlineData("home-n", "Windows OEM activation succeeded.", "InstallProductKey,Activate")]
    [InlineData("home-single-language", "Windows OEM activation succeeded.", "InstallProductKey,Activate")]
    [InlineData("home-country-specific", "Windows OEM activation succeeded.", "InstallProductKey,Activate")]
    [InlineData("pro-n", "Windows OEM activation succeeded.", "InstallProductKey,Activate")]
    [InlineData("addon", "Windows OEM activation succeeded.", "InstallProductKey,Activate")]
    [InlineData("already-oem", "Windows OEM activation succeeded.", "Activate")]
    ...
    [InlineData("missing-installed-product", "installed firmware key could not be verified", "InstallProductKey")]
    [InlineData("generic-active-after-install", "Windows OEM activation succeeded.", "InstallProductKey")]
    [InlineData("generic-active-after-activate", "Windows OEM activation succeeded.", "InstallProductKey,Activate")]
    [InlineData("generic-inactive-after-install", "installed firmware key could not be verified", "InstallProductKey")]
    [InlineData("ambiguous-after-activate", "Windows activation status could not be verified", "InstallProductKey,Activate")]
    [InlineData("offline", "stage=activate; HRESULT=0xC004F074", "InstallProductKey,Activate")]
    [InlineData("pending", "Windows remains unactivated", "InstallProductKey,Activate")]
    public async Task Activation_PreservesLicensingIntentAndReportsSanitizedOutcome(string scenario, string outcome, string actions)
    {
        ...
        Assert.Contains($"ACTIONS=[{actions}]", result.StandardOutput);
    ...
    private const string Harness = """
        ...
        $global:Actions = New-Object 'System.Collections.Generic.List[string]'
            ...
            switch ($ClassName) {
                ...
                'SoftwareLicensingProduct' {
                    ...
                    $product = [pscustomobject]@{
                        ...
                        LicenseStatus = $(if ($Scenario -eq 'activated' -or ($global:Activated -and $Scenario -ne 'pending')) { 1 } else { 0 })
                    ...
                    if ($global:Activated -and $Scenario -eq 'ambiguous-after-activate') { $product.PSObject.Copy() }
        ...
        Write-Output ('ACTIONS=[' + ($global:Actions -join ',') + ']')
        """;
}
