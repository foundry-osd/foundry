...
public sealed class NativeDismImageInfoReaderTests
{
    ...
    public async Task ReadAsync_CopiesPackedArrayWithExactSizesUnicodeAndUndefinedSetupArchitecture()
    {
        ...
        IReadOnlyList<WindowsImageInfo> images = await reader.ReadAsync(@"C:\images\install.esd", TestContext.Current.CancellationToken);

        Assert.Equal(
            [
                new WindowsImageInfo(9, "Windows 11 Éducation 日本語", "Professional", 26_839_601_777, "x64", new Version(10, 0, 26100)),
                new WindowsImageInfo(1, "Windows Setup Media", "", 277_641_908, "unknown", new Version(0, 0, 0)),
                new WindowsImageInfo(4, "Windows ARM64", "Enterprise", 9_223_372_036_854_775_808, "arm64", new Version(10, 1, 22631)),
                new WindowsImageInfo(3, "Windows x86", "Core", 4_294_967_297, "x86", new Version(6, 3, 9600))
            ], images);
    ...
    public async Task ReadAsync_CancellationDuringNativeReadWaitsForReturnAndBufferRelease()
    {
        ...
        using var reader = new NativeDismImageInfoReader(api);
        var returned = new TaskCompletionSource<Task<IReadOnlyList<WindowsImageInfo>>>(TaskCreationOptions.RunContinuationsAsynchronously);
        ...
        Task<IReadOnlyList<WindowsImageInfo>>? read = null;
        try
        {
    ...
    public async Task ReadAsync_ConcurrentReadsAndDisposalDoNotOverlapNativeOwnership()
    {
        ...
        var reader = new NativeDismImageInfoReader(api);
        Task<IReadOnlyList<WindowsImageInfo>> first = reader.ReadAsync("first.esd", TestContext.Current.CancellationToken);
        Task<IReadOnlyList<WindowsImageInfo>>? second = null;
        Task? disposal = null;
        try
        {
            Assert.True(entered.Wait(TestTimeout, TestContext.Current.CancellationToken));
