...
- [ ] No WinUI `Foundry` runtime data is written under `%LocalAppData%` except approved WebView2 user data under `%LOCALAPPDATA%\Foundry\WebView2`.
...
- [ ] Scheduled release is enabled for Tuesday `13:00 UTC` and creates releases only when `src` changes exist or `force` is used.
